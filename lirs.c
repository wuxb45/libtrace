...
get_range(FILE *trc_fp, unsigned long *p_vm_size, unsigned long *p_trc_len)
{
  ...
  printf(" [%ld  %ld] for %lu refs in the trace\n", min, max, count);
  fseek(trc_fp, 0, SEEK_SET);
  *p_vm_size = max;
  *p_trc_len = count;
  return TRUE;
...
main(int argc, char* argv[])
{
  ...
  while (!feof(para_fp)){
    ...
    printf("total blocks refs = %lu  number of misses = %lu \nhit rate = %2.1f%%, mem shortage ratio = %2.1f%% \n",
        total_pg_refs, num_pg_flt, (1.0-(float)num_pg_flt/warm_pg_refs)*100.0, (float)mem_size/vm_size*100.0);

    fprintf(cuv_fp, "%5lu  %2.1f\n", mem_size, 100.0-(float)num_pg_flt/warm_pg_refs*100.0);
    if (sln_fp)
      fclose(sln_fp);
    fscanf(para_fp, "%lu", &mem_size);
  }
