...
{
  const uint32_t nr_keys = lirs->nr_keys;
  assert(lirs->arr[key].next == nr_keys);
  assert(lirs->arr[key].prev == nr_keys);
...
lirs_hir_insert(struct lirs * const lirs, const uint32_t key)
{
  ...
  assert(lirs->arr[key].hprev == nr_keys);
